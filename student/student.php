                    ...
                    <form id="evaluation-form" method="post" action="submit_evaluation.php">
                        ...
                        <?php else: ?>
                            <?php if ($quickEval && !empty($quickEval['faculty_id'])): ?>
                                <div class="success-message">Subject and faculty have been pre-selected from a QR code. Please complete and submit the evaluation.</div>
                            <?php endif; ?>
                            <div class="form-group">
                                ...
                                <select id="enrollment_select" required>
                                    ...
                                    <?php foreach ($enrollments as $en): ?>
                                        <?php
                                            $enFacultyId = (int)$en['faculty_id'];
                                            $enSubCode = trim((string)($en['subject_code'] ?? ''));
                                            $enSubName = trim((string)$en['subject_name']);
                                            $isQuickMatch = $quickEval &&
                                                $quickEval['faculty_id'] === $enFacultyId &&
                                                (
                                                    ($quickEval['subject_code'] !== '' && $quickEval['subject_code'] === $enSubCode) ||
                                                    ($quickEval['subject_code'] === '' && $quickEval['subject_name'] !== '' && strcasecmp($quickEval['subject_name'], $enSubName) === 0)
                                                );
                                        ?>
                                        <option 
                                            ...
                                            <?php echo $isQuickMatch ? 'selected' : ''; ?>
                                        >
